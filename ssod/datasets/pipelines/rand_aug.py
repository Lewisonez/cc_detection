...
weak_prob = 0.4
weak_magnitude = 3
weak_random_magnitude = True
strong_prob = 0.8
strong_magnitude = 9
strong_random_magnitude = True
...
class RandAug(object):
    ...
    def __call__(self, results):
        if results["aug_info"][1]['flip_ratio'] == 0.49:
            self.prob = weak_prob
            self.magnitude = weak_magnitude
            self.random_magnitude = weak_random_magnitude
        if results["aug_info"][1]['flip_ratio'] == 0.51:
            self.prob = strong_prob
            self.magnitude = strong_magnitude
            self.random_magnitude = strong_random_magnitude
...
class GeometricAugmentation(object):
    ...
    def __call__(self, results):
        ...
        if np.random.random() < self.prob:
            ...
            if self.record:
                if "aug_info" not in results:
                    results["aug_info"] = []
                results["aug_info"].append(self.get_aug_info(**magnitude))#将erase的patch信息也加入
            results = self.apply(results, **magnitude)#对results中的patch做像素更改
            self._filter_invalid(results, min_size=self.min_size)#没有实际执行
        return results
    ...
    def get_aug_info(self, **kwargs):
        aug_info = dict(type=self.__class__.__name__)#得到增强的type，如RandErase
        # aug_info.update(#加入除type的一下信息，
        #     dict(
        ...
        aug_info.update(kwargs)#加入了erase内获得的多个patch的信息列表
        return aug_info
    ...
    def get_magnitude(self, results):
        ...
        if self.random_magnitude:
            if isinstance(self.x, (list, tuple)):
                ...
                x = np.random.choice((-1, 1)) * np.random.random() * (self.magnitude / PARAMETER_MAX) * self.x[1]
                magnitude["x"] = x
            if isinstance(self.y, (list, tuple)):
                assert len(self.y) == 2
                # y = np.random.random() * (self.y[1] - self.y[0]) + self.y[0]
                y = np.random.choice((-1, 1)) * np.random.random() * (self.magnitude / PARAMETER_MAX) * self.y[1]
                magnitude["y"] = y
        else:
        ...
        if self.random_magnitude:
            if isinstance(self.angle, (list, tuple)):
                ...
                angle = (
                    ...
                    np.random.choice((-1, 1)) * np.random.random() * (self.magnitude / PARAMETER_MAX) * self.angle[1]
                )
...
class RandErase(GeometricAugmentation):
    ...
    def get_magnitude(self, results):
        magnitude = {}
        if self.random_magnitude:#True
            n_iterations = self._get_erase_cycle()#4
            patches = []
            h, w, c = results["img_shape"]#img_shape=(1166,1166,3),反而ori_shape=(500,500,3)
            for i in range(n_iterations):
                # random sample patch size in the image
                ph, pw = self._get_patch_size(h, w)#ph, pw=197，197
                # random sample patch left top in the image    #px, py=224，958
                px, py = np.random.randint(0, w - pw), np.random.randint(0, h - ph)#假设框在右下角，左上角坐标只能在非框范围内
        ...
        return magnitude#在1166,1166这个范围内

    def _get_erase_cycle(self):
        if isinstance(self.n_iterations, int):#self.n_iterations=（1，5），不是int型
            ...
            n_iterations = np.random.randint(*self.n_iterations)#从（1，5）内返回一个随机int，4
        return n_iterations
    ...
    def _get_patch_size(self, h, w):
        if isinstance(self.size, float):
        ...
        else:#self.size=[0,0.2]，config传进来的参数
            ...
            w_ratio = np.random.random() * (self.magnitude / PARAMETER_MAX) * (self.size[1] - self.size[0]) + self.size[0]
            h_ratio = w_ratio

            if not self.squared:#self.squared也是config参数，直接跳过if
                h_ratio = (
    ...
    def apply(self, results, patches: list):
        for patch in patches:
            self._erase_image(results, patch, fill_val=self.img_fill_val)#img_fill_val是(125.0,125.0,125.0)
            self._erase_mask(results, patch)#没有实际执行
            self._erase_seg(results, patch, fill_val=self.seg_ignore_label)
        return results

    def _erase_image(self, results, patch, fill_val=128):
        for key in results.get("img_fields", ["img"]):#key是'img',对应的是(1166,1166,3)
            tmp = results[key].copy()#将这个(1166,1166,3)的图片复制到temp
            x1, y1, x2, y2 = patch
            tmp[y1:y2, x1:x2, :] = fill_val#将这个patch区域填充为fill_val=(125.0,125.0,125.0),不是128，因为有传入值，不采用默认值
            results[key] = tmp#将results中的image中的patch替换掉

    def _erase_mask(self, results, patch, fill_val=0):
        for key in results.get("mask_fields", []):#   .mask_fields=[] 没有实际执行
            masks = results[key]
    ...
    def _erase_seg(self, results, patch, fill_val=0):
        for key in results.get("seg_fields", []):#   .seg_fields=[]  没有实际执行
            seg = results[key].copy()
            x1, y1, x2, y2 = patch
            ...
            if results["flip"]:
                ...
                results["aug_info"].append(
                    dict(
                        type=self.__class__.__name__,
                        record=False,
                        flip_ratio=self.flip_ratio,#1.0
                        direction=results["flip_direction"],
                    )
                )
            else:
                results["aug_info"].append(
                    dict(
                        type=self.__class__.__name__,
                        record=False,
                        flip_ratio=self.flip_ratio,#0.0
                        direction="vertical",
                    )
                )
        return results
...
class MultiBranch(object):
    ...
    def __call__(self, results):
        multi_results = []
        for k, v in self.transform_group.items():#两项：unsup_student,  unsup_teacher,分别有自己的transforms
            res = v(copy.deepcopy(results))
            if res is None:
                return None
            # res["img_metas"]["tag"] = k
            multi_results.append(res)
        return multi_results#两个项目，包含图片和标注信息，
