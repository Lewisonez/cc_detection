...
class Resize:
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                ...
                new_h, new_w = img.shape[:2]#计算出新的图片的w和h
                h, w = results[key].shape[:2]#原始图片的h和w
                w_scale = new_w / w#相除算出缩放因子
                h_scale = new_h / h
            else:
            ...
            scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                    dtype=np.float32)
            results['img_shape'] = img.shape#图片由(500,500,3)变成了(475,475,3)
            # in case that there is no padding
            results['pad_shape'] = img.shape#和上边一样
    ...
    def __call__(self, results):
        ...
        self._resize_img(results)#把img_shape变成了(475,475,3)
